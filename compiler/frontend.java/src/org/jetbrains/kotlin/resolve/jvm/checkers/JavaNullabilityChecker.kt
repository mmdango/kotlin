...
class JavaNullabilityChecker : AdditionalTypeChecker {
    ...
    override fun checkReceiver(receiverParameter: ReceiverParameterDescriptor, receiverArgument: ReceiverValue, safeAccess: Boolean, c: CallResolutionContext<*>) {
        ...
        doCheckType(
            ...
            actualType ->
            val expressionReceiver = (receiverArgument as? ExpressionReceiver)?.expression
            if (expressionReceiver != null) {
                c.trace.report(ErrorsJvm.RECEIVER_NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS.on(expressionReceiver, actualType))
            } else {
