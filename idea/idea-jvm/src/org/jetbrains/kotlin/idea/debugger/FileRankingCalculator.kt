...
import org.jetbrains.kotlin.idea.debugger.FileRankingCalculator.Ranking.Companion.SINGLE
import org.jetbrains.kotlin.idea.debugger.FileRankingCalculator.Ranking.Companion.DOUBLE
...
abstract class FileRankingCalculator(
    ...
    private class Ranking(val value: Int) : Comparable<Ranking> {
        companion object {
            ...
            val ZERO = Ranking(0)
            val SINGLE = Ranking(1)
            val DOUBLE = Ranking(2)

            fun of(condition: Boolean) = if (condition) SINGLE else ZERO
            fun double(condition: Boolean) = if (condition) DOUBLE else ZERO
        }
    ...
    private fun collect(vararg conditions: Any): Ranking {
        return conditions
            .map { condition ->
                when (condition) {
                    is Boolean -> Ranking.of(condition)
    ...
    private fun rankingForClass(clazz: KtClassOrObject, type: ReferenceType): Ranking {
        ...
        val nameRanking = rankingForClassName(type.name(), descriptor, bindingContext)
            .takeIf { it.value >= 0 } ?: return LOW

        return collect(
            nameRanking,
            Ranking.double(type.isAbstract && descriptor.modality == Modality.ABSTRACT),
            Ranking.double(type.isFinal && descriptor.modality == Modality.FINAL),
            Ranking.double(type.isStatic && !descriptor.isInner),
    ...
    private fun rankingForClassName(fqName: String, descriptor: ClassDescriptor, bindingContext: BindingContext): Ranking {
        ...
        return when {
            checkClassFqName -> if (expectedFqName == fqName) DOUBLE else -SINGLE
            else -> if (expectedFqName.simpleName() == fqName.simpleName()) DOUBLE else -SINGLE
        }
    }
    ...
    private fun rankingForProperty(property: KtProperty, method: Method): Ranking {
        ...
        // boolean is
        return if (methodName.drop(3) == propertyName.capitalize()) SINGLE else -SINGLE
    }
    ...
    private fun rankingForType(ktType: KotlinType?, jdiType: Type?, typeMapper: KotlinTypeMapper): Ranking {
        ...
        if (ktType == null)
            return Ranking.of(jdiType is VoidType)
        ...
        if (method.isLambda()) {
            ...
            if (method.isBridge && method.isSynthetic) {
                ...
                when (containingCallable) {
                    is KtFunction -> if (containingCallable.name == methodName) overallRanking += SINGLE
                    is KtProperty -> if (containingCallable.name == methodName) overallRanking += SINGLE
                    is KtPropertyAccessor -> if (containingCallable.property.name == methodName) overallRanking += SINGLE
                }
